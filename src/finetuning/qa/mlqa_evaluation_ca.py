...
#
""" Modified version of the official evaluation script for the MLQA dataset extendend to support Catalan"""
...
PUNCT = {chr(i) for i in range(sys.maxunicode) if unicodedata.category(chr(i)).startswith('P')}.union(
    string.punctuation)
WHITESPACE_LANGS = ['en', 'es', 'ca', 'hi', 'vi', 'de', 'ar']
...
def normalize_answer(s, lang):
    ...
    def remove_articles(text, lang):
        if lang == 'en':
            ...
            return re.sub(r'\b(un|una|uns|unes|el|la|els|les|l\')\b', ' ', text)
        elif lang == 'hi':
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Evaluation for Catalan')
    parser.add_argument('--testset', required=True, help='Dataset file')
    parser.add_argument('--prediction_file', required=True, help='Prediction File')
    parser.add_argument('--answer_language', required=True, help='Language code of answer language')

    args = parser.parse_args()
    with open(args.testset) as ts:
        testset = json.load(ts)
        dataset = testset['data']
