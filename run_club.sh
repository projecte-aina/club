...
bash $SCRIPT_DIR/src/pos/run_pos.sh ${model_name} 2>&1 | tee -a $SCRIPT_DIR/run_club-${model_name}.log

## Get results
{ echo "POS";
  echo -n "F1 = ";
  grep  -Po "(?<=predict_f1\": )[0-9]\.[0-9]+" $SCRIPT_DIR/src/pos/${model_name}-pos/predict_results.json ; } \
  | tee  $SCRIPT_DIR/results-${model_name}


# Named Entity Recognition
echo "Task: Named Entity Recognition"
bash $SCRIPT_DIR/src/ner/run_ner.sh ${model_name} 2>&1 | tee -a $SCRIPT_DIR/run_club-${model_name}.log

## Get results
{ echo -e "\nNER";
  echo -n "F1 = ";
  grep  -Po "(?<=predict_f1\": )[0-9]\.[0-9]+" $SCRIPT_DIR/src/ner/${model_name}-ner/predict_results.json ; } \
  | tee -a $SCRIPT_DIR/results-${model_name}


# Semantic Textual Similarity
echo "Task: Semantic Textual Similarity"
bash $SCRIPT_DIR/src/sts/run_sts.sh ${model_name} 2>&1 | tee -a $SCRIPT_DIR/run_club-${model_name}.log
...
bash $SCRIPT_DIR/src/tc/run_tc.sh ${model_name} 2>&1 | tee -a $SCRIPT_DIR/run_club-${model_name}.log
...
bash $SCRIPT_DIR/src/te/run_te.sh ${model_name} 2>&1 | tee -a $SCRIPT_DIR/run_club-${model_name}.log
...
bash $SCRIPT_DIR/src/qa/run_qa.sh ${model_name} 2>&1 | tee -a $SCRIPT_DIR/run_club-${model_name}.log
